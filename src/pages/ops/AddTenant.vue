  ...
  <div class="add-new-member">
    <div class="popup">
      ...
      <div class="form-item">
        ...
        <el-card style="margin-top: 6px;max-height: 160px;overflow-y: auto">
          <el-checkbox-group v-model="selectedTenant">
            <el-checkbox v-for="item in tenantList"
                         :label="item.id"
                         :key="item.id"
...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then((res) => {
          ...
          this.tenantList = this.tenantList.filter(item => {
            return item.roles.includes(UserType.tenantAdmin) && item.distributorId === null && item.stripeCustomerId === null;
          });
        })
        .finally(() => {
    ...
    handleAdd() {
      this.$psPageLoading().show();
      this.$http.addTenant(this.currentClient.id, this.selectedTenant)
        .then(() => {
          this.$emit('confirmAddTenantSuccess');
        }).finally(() => {
          this.$psPageLoading().hide();
        });
    }
