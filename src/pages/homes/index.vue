  ...
  <div class="homes__wrapper">
    ...
    <div class="table__container">
      ...
      >
        ...
        <el-table-column prop="rooms" min-width="150">
          <template slot="header">
            <div class="table-header-sort">
              <div class="table-header-text">{{ $t("associatedRooms") }}</div>
            </div>
          </template>
          <template slot-scope="scope">
            <div
              class="amountNumber"
              @click="openNameList(scope.row.rooms, 'room')"
              v-if="scope.row.rooms && scope.row.rooms.length > 0"
            >
        ...
        <el-table-column prop="users" min-width="150">
          ...
          <template slot-scope="scope">
            <div
              class="amountNumber"
              @click="openNameList(scope.row.users, 'member')"
              v-if="scope.row.users && scope.row.users.length > 0"
            >
...
export default {
  ...
  methods: {
    ...
    confirmExport() {
      this.$psPageLoading().show();
      this.$http.exportHomeList({
        ...(this.lastFilterSnapshot || {}),
        order: this.order,
        sort: this.sort
      }).then((res) => {
        this.blobUtil(res, `Homes-${moment(new Date()).format('YYYY-MM-DD')}.xlsx`);
