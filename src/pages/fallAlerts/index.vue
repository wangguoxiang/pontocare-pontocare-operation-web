<template>
  <div class="fall-alerts__wrapper">
    ...
    <div class="table__container">
      ...
      >
        ...
        <el-table-column prop="location" width="160">
          <template slot="header">
            <div class="table-header-sort">
              <div class="table-header-text">{{ $t("location") }}</div>
            </div>
          </template>
          <template slot-scope="scope">
            <PsFallArea :key="scope.row.eventId"
                        :fallInfo="scope.row.data"
                        :roomName="scope.row.roomName"
                        :roomType="scope.row.roomType"
                        :homeName="scope.row.homeName"
                        :happenedTime="scope.row.happenedTime"
                        :duration="scope.row.duration"
                        :isShowInfo="false"
            ></PsFallArea>
          </template>
        </el-table-column>
        <el-table-column prop="eventId" min-width="120">
          <template slot="header">
            <div class="table-header-sort">
              <div class="table-header-text">{{ $t("alertID") }}</div>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="sn" min-width="120">
        ...
        <el-table-column prop="tenantName" min-width="150">
          <template slot="header" slot-scope="scope">
            <div class="table-header-sort">
              <div class="table-header-text">
                {{ $t("organization") }}
        ...
        <el-table-column prop="homeName" min-width="150">
          <template slot="header" slot-scope="scope">
            <div class="table-header-sort">
              <div class="table-header-text">
                {{ $t("home") }}
        ...
        <el-table-column prop="roomName" min-width="150">
          <template slot="header" slot-scope="scope">
            <div class="table-header-sort">
              <div class="table-header-text">
                {{ $t("room") }}
        ...
        <el-table-column prop="happenedTime" min-width="150">
          <template slot="header" slot-scope="scope">
            <div class="table-header-sort">
              <div class="table-header-text">
                {{ $t("time") }}
          ...
          <template slot-scope="scope">
            <div>{{ formatTableTime(scope.row.happenedTime) }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="duration" min-width="100">
          <template slot="header">
            <div class="table-header-sort">
              <div class="table-header-text">{{ $t("fallDuration") }}</div>
            </div>
          </template>
          <template slot-scope="scope">
            <div>{{ formatAlertDuration(scope.row.duration) }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="mark" width="120">
          <template slot="header">
            <div class="table-header-sort">
              <div class="table-header-text">{{ $t("mark") }}</div>
            </div>
          </template>
          <template slot-scope="scope">
            <div>{{ scope.row.isTp === null ? '-' : markMap[scope.row.isTp] }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="note" width="150">
          <template slot="header">
            <div class="table-header-sort">
          ...
          <template slot-scope="scope">
            <div v-if="scope.row.note && isJsonString(scope.row.note)"
                 class="yes-content"
                 @click="showContent(scope.row.note)">{{$t("noteAdded")}}</div>
            <ps-tool-tip v-else-if="scope.row.note" :content="scope.row.note"></ps-tool-tip>
            <div v-else>-</div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <YesContent v-if="showYesContent" @cancel="showYesContent = false" :yesNote="currentContent"></YesContent>
    <PsConfirm
      v-if="showConfirm"
...
export default {
  ...
  data() {
    return {
      ...
      disabledDates: {
        // 禁用的自定义规则
        from: moment().add(0, 'day').startOf('day').toDate() // 禁用明天及以后的日期
      },
      startDate: null,
      endDate: null,
  ...
  methods: {
    ...
    initDate() {
      ...
      startOfDay.setDate(now.getDate() - 6);
    ...
    getFallAlertList() {
      ...
      this.$http
        .getFallAlertList(filterParams)
        .then((res) => {
          this.homeList = res.records;
    ...
    confirmExport() {
      this.$psPageLoading().show();
      this.$http.exportFallAlertList({
        ...(this.lastFilterSnapshot || {}),
        order: this.order,
        sort: this.sort
      }).then((res) => {
        this.blobUtil(res, `FallAlerts-${moment(new Date()).format('YYYY-MM-DD')}.xlsx`);
    ...
    showContent(content) {
      this.currentContent = JSON.parse(content);
      this.showYesContent = true;
    },
